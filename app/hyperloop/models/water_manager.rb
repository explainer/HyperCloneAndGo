...
# FREESTANDER = "#{Rails.root.to_s}/lib/tasks/freestander"
...
class WaterManager < ApplicationRecord
  ...
  def manipulate_and_update(params, request)
    ...
    if host_with_port.to_s.empty?
      ...
      update(params)
      #logger.info "params['http_host'] --> #{params['http_host']}"
    end
    # set crontab state
    #logger.info "water_manager_params --> #{params.inspect}"
    if "active".start_with?(params["state"].downcase)
      state = "Active"
      arm
    else # state == "Standby"
      state = "Standby"
    ...
    params["state"] = state
    #logger.info "WaterManager.manipulate_and_update(#{params})"
    update(params)
    true
  end
